...
export class VendorLibraries extends VendorLibrariesBase {
  ...
  public async manageVendorLibraries(uri: vscode.Uri | undefined): Promise<void> {
    ...
    this.getCachedHomeDirDeps(false).catch(err => { // false: do not force refresh from disk if cache is valid
      logger.warn('Failed to preload home directory dependencies for vendor libraries', { error: err });
    });
  ...
  public async getCachedHomeDirDeps(forceRefresh: boolean = false): Promise<IJsonDependency[]> {
    ...
    if (!forceRefresh && this.homeDirDepsCache && (now - this.homeDirDepsCacheLastUpdated < CACHE_DURATION_MS)) {
          ...
          for (const ti of toRemove) {
            if (result.uuid === ti.uuid) {
              try {
                ...
                logger.error('Failed to delete vendor dependency file', { file: fullPath, error: err });
              }
            }
          }
        }
      ...
      for (const dep of existing) {
        if (dep.uuid === file.uuid) {
          if (dep.version === file.version) {
            vscode.window.showInformationMessage(i18n('message', '{0} version {1} is already installed.', file.name, file.version));
            return;
          } else {
            const res = await vscode.window.showWarningMessage(i18n('message',
              '{0} version {1} is already installed. Would you like to update to {2}?',
              file.name, dep.version, file.version), { modal: true }, i18n('ui', 'Yes'), i18n('ui', 'No'));
            if (res !== i18n('ui', 'Yes')) {
              return;
            }
          }
  ...
  private async getInstalledDependencies(workspace: vscode.WorkspaceFolder): Promise<IJsonDependency[]> {
